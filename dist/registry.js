...
export class SkillsRegistry {
     ...
     */
    async loadFromConfigFile(filePath) {
     ...
     * @returns Validation result.
     */
    async validateSkillByName(name) {
    ...
    async validatePlugins() {
        for (const plugin of this.plugins) {
            ...
            for (const serverName of declaredMcpServers) {
                ...
                const discoveredOutsidePlugin = this.mcpServers.some((server) => server.name === serverName && !isSubpath(plugin.rootDir, server.sourcePath));
                if (!discoveredOutsidePlugin) {
                    this.diagnostics.push({
                        ...
                        file: plugin.sourcePath,
                        message: "Plugin bundles an MCP server that is not present in the discovered project config; this is allowed but should be reviewed.",
    ...
    async resolvePluginSkillReferences(plugin) {
...
function parseStructuredConfig(content, filePath) {
...
function dedupeMcpServers(servers) {
    const seen = new Set();
    const deduped = [];
...
function relativizeRegistryIndex(index, cwd) {
...
function relativizePathValue(value, root) {
    if (!value) {
        return undefined;
    }
...
function issueLocation(issue, cwd) {
    if (!issue.file) {
        return undefined;
    }
    const file = cwd && path.isAbsolute(issue.file)
...
function issueDisplayPath(issue, cwd) {
